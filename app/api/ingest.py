...
from app.models.base import IngestRequest, IngestResponse, ErrorResponse
...
class IngestService:
    ...
    async def check_document_exists(self, content_hash: str, db) -> Dict[str, Any]:
        ...
        try:
            result = db.execute(
                text("SELECT doc_id, title, total_chunks FROM documents WHERE content_hash = :hash"),
                {"hash": content_hash}
            ).fetchone()
    ...
    async def delete_missing_documents(self, provided_paths: List[str], db) -> int:
        ...
        try:
            ...
            result = db.execute(
                text("SELECT doc_id, source_path FROM documents")
            ).fetchall()
...
async def ingest_documents(
    ...
    try:
        ...
        if unsupported_paths:
            raise HTTPException(
                status_code=415,
                detail=f"Unsupported file formats: {unsupported_paths}"
            )
