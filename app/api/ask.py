...
class RAGPipeline:
    ...
    async def process_question(self, question: str) -> dict[str, Any]:
        ...
        try:
            ...
            avg_score = (
                sum(r.get("score", 0) for r in reranked_results) / len(reranked_results)
                if reranked_results
                else 0
            )
...
async def ask_question(request: AskRequest):
    ...
    try:
        logger.info(
            "Processing question", question_length=len(request.question), trace_id=get_trace_id()
        )
        ...
        if len(question) < 5:
            raise HTTPException(
                status_code=400, detail="Question is too short (minimum 5 characters)"
            )

        if len(question) > 500:
            raise HTTPException(
                status_code=400, detail="Question is too long (maximum 500 characters)"
            )
        ...
        logger.error("Question processing failed", error=str(e), trace_id=get_trace_id())
        ...
        if "timeout" in error_msg:
            ...
            raise HTTPException(status_code=503, detail="External service connection failed")
        else:
...
async def ask_health_check():
    ...
    try:
        ...
        return {"status": overall_status, "components": health_checks, "timestamp": time.time()}
