...
from app.models.base import EvalRunRequest, EvalRunResponse, EvaluationCase, EvaluationResult
        ...
        try:
            ...
            db.execute(
                ...
                """),
                {
                    ...
                    "status": "completed" if results.get("failed_cases", 0) == 0 else "partially_failed",
            ...
            for case_result in results.get("case_results", []):
                ...
                db.execute(
                    ...
                    """),
                    {
                        ...
                        "context_recall": None,
                        "flags": json.dumps([]),
                        "errors": json.dumps({"error": case_result.get("error")} if case_result.get("error") else {})
                    }
                )
...
async def run_evaluation(
    ...
    try:
        ...
        if not request.cases and not request.file_path:
            raise HTTPException(
                status_code=400,
                detail="Either 'cases' or 'file_path' must be provided"
            )
        
        if request.cases and request.file_path:
            raise HTTPException(
                status_code=400,
                detail="Provide either 'cases' or 'file_path', not both"
            )
        ...
        if request.file_path:
            try:
                ...
                raise HTTPException(
                    status_code=400,
                    detail=f"Failed to load test cases from file: {str(e)}"
                )
        else:
        ...
        if not test_cases:
            raise HTTPException(
                status_code=400,
                detail="No valid test cases found"
            )
        
        if len(test_cases) > 1000:
            raise HTTPException(
                status_code=400,
                detail="Too many test cases (maximum 1000)"
            )
...
async def get_evaluation_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to retrieve evaluation status"
        )