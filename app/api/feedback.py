...
class FeedbackService:
    ...
    def __init__(self):
        ...
        self.valid_reasons = feedback_config.get("reasons", [
            "галлюцинация",
            "не по теме",
            "неполный ответ", 
...
async def submit_feedback(
    ...
    try:
        ...
        if len(request.llm_answer) > 5000:
            raise HTTPException(
                status_code=400,
                detail="Answer is too long (maximum 5000 characters)"
            )
        
        if request.comment and len(request.comment) > 1000:
            raise HTTPException(
                status_code=400,
                detail="Comment is too long (maximum 1000 characters)"
            )
        
        # Save feedback
        feedback_id = await feedback_service.save_feedback(
            request, client_ip, user_agent, db
        )
