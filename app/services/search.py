...
class ElasticsearchService:
    ...
    def __init__(self):
        ...
        self.client = AsyncElasticsearch(
            [settings.elasticsearch_url], timeout=30, retry_on_timeout=True
        )
    ...
    async def ensure_index_exists(self):
        ...
        try:
            ...
            if not exists:
                ...
                mapping = {
                    ...
                    "settings": {
                        ...
                        "analysis": {
                            "analyzer": {"standard": {"type": "standard", "stopwords": "_english_"}}
                        },
                    },
                }
                ...
                self.logger.log_operation(
                    "create_index", duration_ms, success=True, index_name=self.index_name
                )
            else:
                duration_ms = (time.time() - start_time) * 1000
                self.logger.log_operation(
            ...
            duration_ms = (time.time() - start_time) * 1000
            self.logger.log_operation(
                "ensure_index", duration_ms, success=False, error=str(e), index_name=self.index_name
            )
            raise
    ...
    async def search_chunks(
        self, query: str, top_k: Optional[int] = None, doc_filter: Optional[str] = None
    ) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if doc_filter:
                search_body["query"]["bool"]["filter"] = [{"term": {"doc_id": doc_filter}}]
...
class HybridSearchService:
    ...
    def normalize_scores(
        self, results: list[dict[str, Any]], max_score: float = 1.0
    ) -> list[dict[str, Any]]:
    ...
    async def search(
        self, query: str, doc_filter: Optional[str] = None
        ...
        try:
            ...
            bm25_task = self.es_service.search_chunks(
                query=query, top_k=self.bm25_top_k, doc_filter=doc_filter
            )

            dense_task = self.vector_service.search_similar_chunks(
                query_text=query, top_k=self.dense_top_k, doc_filter=doc_filter
            )
