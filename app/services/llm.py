...
class OllamaService:
    ...
    def __init__(self):
        ...
        self.max_context_length = generation_config.get(
            "max_context_length", settings.context_window
        )
        self.citation_format = generation_config.get(
    ...
    async def pull_model(self) -> bool:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=300.0) as client:  # 5 minutes for model pull
                ...
                self.logger.log_operation("pull_model", 0, success=True, model_name=self.model_name)

                return True

        except Exception as e:
            self.logger.log_operation(
                "pull_model", 0, success=False, error=str(e), model_name=self.model_name
            )
            return False
