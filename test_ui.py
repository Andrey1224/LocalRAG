        ...
        if st.button("🚀 Загрузить файлы в систему", type="primary"):
            ...
            for i, file in enumerate(uploaded_files):
                ...
                try:
                    ...
                    # Загружаем через API
                    data = {"paths": [temp_path], "delete_missing": False}
                    response = requests.post(f"{API_BASE_URL}/ingest", json=data, timeout=60)

                    if response.status_code == 200:
                        result = response.json()
                        successful_uploads.append({"name": file.name, "result": result})
                    else:
                    ...
                    failed_uploads.append({"name": file.name, "error": str(e)})
            ...
            if successful_uploads:
                ...
                with st.expander("📊 Детали успешных загрузок"):
                    for upload in successful_uploads:
                        st.write(
                            f"**{upload['name']}**: {upload['result']['indexed']} чанков проиндексировано"
                        )
            ...
            if successful_uploads:
                st.info(
                    "💡 Теперь вы можете перейти в раздел 'Ask Test' для задания вопросов к загруженным документам!"
                )
...
def test_ingest():
    ...
    document_path = st.text_input(
        "Путь к документу:", value="/Users/nepodymka/Desktop/LocalRAG/test_document.md"
    )
    ...
    if st.button("Загрузить документ"):
        if document_path:
            with st.spinner("Загружаем документ..."):
                try:
                    data = {"paths": [document_path], "delete_missing": False}
                    response = requests.post(f"{API_BASE_URL}/ingest", json=data, timeout=60)
...
def test_ask():
    ...
    question = st.text_input("Ваш вопрос:", value="Какие меры безопасности описаны в политике?")
    ...
    with col3:
        if st.button("🔍 Debug API"):
            ...
            st.json(
                ...
                else {"error": debug_response.status_code}
            )
        ...
        if response.status_code == 200:
            ...
            if data["documents"]:
                ...
                for doc in data["documents"]:
                    with st.expander(
                        f"📄 {doc['filename']} ({doc['size']} байт, {doc['chunks']} чанков)"
                    ):
                        ...
                        with col2:
                            if st.button("🗑️ Удалить", key=f"delete_{doc['doc_id']}"):
                                # Подтверждение удаления
                                if st.button(
                                    f"✅ Подтвердить удаление {doc['filename']}",
                                    key=f"confirm_{doc['doc_id']}",
                                ):
                                    with st.spinner("Удаляем документ..."):
                                        try:
                                            ...
                                            if delete_response.status_code == 200:
                                                ...
                                                st.write(
                                                    f"Удалено чанков: {result['deleted_chunks']}"
                                                )
                                                st.rerun()
                                            else:
                                                st.error(
                                                    f"❌ Ошибка удаления: {delete_response.status_code}"
                                                )
